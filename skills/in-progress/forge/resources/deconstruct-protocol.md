> Bundled resource — the forge's Movement I deconstruction. The one genuinely unique piece.

# Desmonte — break a method into function, not surface

Adapts the deconstruction core of the `leonardo-method` study method with its **polarity inverted**: there a human disassembles external knowledge to internalise it (the coach withholds); here the system disassembles the *expert's* method into a machine artifact (it actively analyses and proposes). Credits that method as origin; does **not** invoke it (wrong polarity). Communicate any expert-facing output in the expert's own language.

## The one discipline: function, not surface sequence

The failure this prevents: a skill that mirrors the *order the expert happens to work in*, copying gestures without grasping why — works on the examples, shatters everywhere else. Get underneath the observed sequence to each step's **function** and its **true dependencies**. The expert's order is often habit, not constraint. Only functional decomposition generalises — and only that lets the reconstruction match or exceed the expert.
...
**Breaks if absent:** <what fails downstream>
**Consumes / Produces:** <inputs / outputs>
...
Studying material to internalise it yourself → use `/leonardo-method` (that polarity is the one you want). No repeatable method underneath → nothing to decompose. Only want a process summary → not this.
